...
export RCFILES_DIR="/opt/synergy/rcfiles"
export SYNERGY_DIST="${SYNERGY_DIST:-stable}"

export PKGS_DIR="/opt/synergy/pkgs"
export SYSROOT_DIR="/opt/synergy/sysroot"

export APOLLO_PROFILE="/etc/profile.d/synergy.sh"
export APOLLO_LD_FILE="/etc/ld.so.conf.d/synergy.conf"
export DOWNLOAD_LOG="/opt/synergy/build.log"
...
function sudo_apt_update_and_install() 
{
    # --fix-missing
...
function apollo_environ_setup() 
{
    ...
    if [ ! -f "${DOWNLOAD_LOG}" ]; then
        echo "##==== Summary: Synergy Package Downloads ====##" > "${DOWNLOAD_LOG}"
...
function download_if_not_cached 
{
    ...
    echo -e "${pkg_name}\t${expected_cs}\t${url}" | sudo tee -a "${DOWNLOAD_LOG}"
    ...
    if [[ $use_cache -eq 0 ]]; then
        ...
        if [ "$my_schema" = "http" ]; then
            ...
            sudo mv -f "${ARCHIVE_DIR}/${pkg_name}" "${pkg_name}"
        elif [ "$my_schema" = "git" ]; then
            sudo tar xzf "$ARCHIVE_DIR/${pkg_name}"
        else
            error "Unknown schema for package \"$pkg_name\", url=\"$url\""
        fi
    fi
